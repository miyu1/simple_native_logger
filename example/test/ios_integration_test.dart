...
void main() {
  test("ios integration test", () async {
    ...
    final deviceName = await findIOSDevice();
    debugPrint("ios simulator: $deviceName");
  ...
  test("useIsLoggable property has no effect on ios", () async {
    const tag = "Stub";
    ...
    final deviceName = await findIOSDevice();
    debugPrint("ios device: $deviceName");
    expect(deviceName, isNotEmpty, reason: "run in ios.");
  ...
  test("ios release build (no stdout echo)", () async {
    ...
    expect(deviceName, isNotEmpty, reason: "connect any ios simulator.");
